...
-- -----------------------------------------------------
-- Table `ejemplo_producto`.`PROVEEDORES`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `ejemplo_producto`.`PROVEEDORES` ;

CREATE TABLE IF NOT EXISTS `ejemplo_producto`.`PROVEEDORES` (
...
CREATE TABLE IF NOT EXISTS `ejemplo_producto`.`PRODUCTO` (
  ...
  PRIMARY KEY (`codigo`),
  INDEX `fk_PRODUCTO_PROVEEDORES1_idx` (`PROVEEDORES_nif` ASC) VISIBLE,
  CONSTRAINT `fk_PRODUCTO_PROVEEDORES1`
    FOREIGN KEY (`PROVEEDORES_nif`)
    REFERENCES `ejemplo_producto`.`PROVEEDORES` (`nif`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
